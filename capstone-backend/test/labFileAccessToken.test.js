...
test('mobile medical links use the configured API origin and bind result access', () => {
  ...
  assert.equal(url.pathname, '/api/lab-results/file/mobile');
  assert.equal(url.searchParams.get('id'), '21');
  assert.equal(verifyLabFileAccessToken(url.searchParams.get('token'), { resultId: '21', now: values.now, env }).pid, 'patient-1');
  assert.equal(verifyLabFileAccessToken(url.searchParams.get('token'), { resultId: '22', now: values.now, env }), null);
});
